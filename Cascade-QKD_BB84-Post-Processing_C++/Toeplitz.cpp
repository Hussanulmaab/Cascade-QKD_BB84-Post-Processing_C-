...
void Toeplitz() {
    ...
    pName = PyUnicode_DecodeFSDefault("Toeplitz");
    if (pName == NULL) {
        PyErr_Print();
    ...
    if (pModule != NULL) {
        ...
        pFunc = PyObject_GetAttrString(pModule, "Func_Toeplitz");
        if (pFunc && PyCallable_Check(pFunc)) {
            ...
            pValue = PyObject_CallObject(pFunc, NULL);
                ...
                Py_DECREF(pModule);
                PyErr_Print();
                throw runtime_error("Call to Func_Toeplitz() failed");
            }
        }
        else {
            if (PyErr_Occurred())
                PyErr_Print();
            throw runtime_error("Cannot find function Func_Toeplitz");
        }
        Py_XDECREF(pFunc);
        Py_DECREF(pModule);
    }
        ...
        PyErr_Print();
        throw runtime_error("Failed to load Toeplitz");
    }
